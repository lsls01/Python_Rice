...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day <= days_in_simulation:
        ...
        previous_earnings = days_vs_earnings[-1][1]
        current_savings = previous_earnings - total_bribe_cost

        # check whether we have enough money to bribe without waiting
        if current_savings >= bribe_cost:
            days_to_bribe = 0
        else:
            days_to_bribe = int(math.ceil(1.0 * (bribe_cost - current_savings) / salary))
        
        # advance current_day to day of next bribe (DO NOT INCREMENT BY ONE DAY)
        current_day += days_to_bribe
        
        # calculate current earnings before bribe
        current_earnings = previous_earnings + (salary * (current_day - days_vs_earnings[-1][0]))
